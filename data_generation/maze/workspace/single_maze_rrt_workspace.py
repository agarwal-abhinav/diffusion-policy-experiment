...
from data_generation.motion_planners.maze_rrt import MazeRRT

class SingleMazeRRTWorkspace:
    ...
    def generate_data(self):
        ...
        maze_rrt = MazeRRT(self.maze, self.source)
        if not self.reset_every_trajectory:
            maze_rrt.grow(N=self.max_samples)
        ...
        while len(data) < self.num_trajectories:
            ...
            if self.reset_every_trajectory:
                maze_rrt.reset()
                path = maze_rrt.grow_to_goal(goal,
                            max_samples=self.max_samples,
                            num_shortcut_attempts=self.num_shortcut_attempts)
            else:
                path = maze_rrt.find_path(goal, 
                            num_shortcut_attempts=self.num_shortcut_attempts)
