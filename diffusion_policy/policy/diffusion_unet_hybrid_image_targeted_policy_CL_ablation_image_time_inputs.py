...
_RobomimicCropRandomizer = getattr(rmobsc, 'CropRandomizer', None) or getattr(rmbn, 'CropRandomizer'), dict_apply_with_exclude
import dill
import hydra
...
class DiffusionUnetHybridImageTargetedPolicy(BaseImagePolicy):
    def __init__(self, 
            ...
            zero_init_trainable_input_bias=False,
            trainable_input_bias=False,
            # parameters passed to step
            **kwargs):
        super().__init__()

        assert (trainable_input_bias and zero_init_trainable_input_bias) == False
        ...
        if obs_as_global_cond:
            ...
            global_cond_dim = obs_feature_dim * n_obs_steps + one_hot_encoding_dim + obs_feature_dim * (max_obs_steps_for_ablation - n_obs_steps)
        ...
        if zero_init_trainable_input_bias: 
            self.input_bias = nn.Parameter(
                torch.zeros(1, max_obs_steps_for_ablation, 1, 1, 1)
            )
        elif trainable_input_bias: 
            self.input_bias = nn.Parameter(
                torch.randn(1, max_obs_steps_for_ablation, 1, 1, 1) * (0.02) - 0.01
            )
        else: 
            input_bias = torch.arange(max_obs_steps_for_ablation)
            angles = 2 * math.pi * input_bias / max_obs_steps_for_ablation
            input_bias = torch.sin(angles).reshape(1, max_obs_steps_for_ablation, 1, 1, 1)
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor], use_DDIM=False) -> Dict[str, torch.Tensor]:
        ...
        if self.obs_as_global_cond:
            ...
            this_nobs = dict_apply_with_exclude(this_nobs, 
                                                lambda x: x + self.input_bias[:, -self.n_obs_steps:, ...], 
                                                ignore_keys=['agent_pos'])
            this_nobs = dict_apply(this_nobs,
