...
class DiffusionUnetHybridImageTargetedPolicy(BaseImagePolicy):
    def __init__(self, 
        ...
        obs_key_shapes_to_pass = copy.deepcopy(obs_key_shapes)
        for key in obs_key_shapes_to_pass.keys(): 
            obs_key_shapes_to_pass[key][0] *= n_obs_steps

        config["observation"]["encoder"]["rgb"]["core_kwargs"]["feature_dimension"] *= n_obs_steps
        config["observation"]["encoder"]["rgb"]["core_kwargs"]["pool_kwargs"]["num_kp"] *= n_obs_steps
        # breakpoint()
        # load model
        policy: PolicyAlgo = algo_factory(
                algo_name=config.algo_name,
                config=config,
                obs_key_shapes=obs_key_shapes_to_pass,
                ac_dim=action_dim,
        ...
        if obs_as_global_cond:
            ...
            global_cond_dim = obs_feature_dim + one_hot_encoding_dim + int(obs_feature_dim/n_obs_steps) * (max_obs_steps_for_ablation - n_obs_steps)
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor], use_DDIM=False) -> Dict[str, torch.Tensor]:
        ...
        if self.obs_as_global_cond:
            ...
            this_nobs = dict_apply(nobs, 
                lambda x: x[:,:self.n_obs_steps,...].reshape(B, -1, *x.shape[3:]))
    ...
    def compute_obs_embedding(self, batch):
        ...
        if self.obs_as_global_cond:
            # reshape B, T, ... to B*T
            this_nobs = dict_apply(nobs, 
                lambda x: x[:,:self.n_obs_steps,...].reshape(batch_size, -1, *x.shape[3:]))
            global_cond = self.obs_encoder(this_nobs)
            if self.project_obs_embedding:
                nobs_features = self.obs_embedding_projector(nobs_features)
            # reshape back to B, Do
