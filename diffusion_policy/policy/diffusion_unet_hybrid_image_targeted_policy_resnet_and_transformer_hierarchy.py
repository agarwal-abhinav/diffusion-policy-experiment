...
def make_summary_context_mask(
    num_summary: int,
    num_context: int,
    attend_counts: List[int],
    causal_context: bool = False,
) -> torch.Tensor:
    """
    Build a mask for a sequence of `K + N` tokens:
      [ summary_0, summary_1, ..., summary_{K-1}, context_0, ..., context_{N-1} ]

    Mask rules (mask[q,k] = True means "block attention from query q to key k"):

    1) summary queries:
      - summary_0 sees ALL summary *and* ALL context.
      - for i=1..K-1, summary_i sees ALL summary *and* the first
        attend_counts[i-1] context tokens, blocks other context tokens.

    2) context queries:
      - always block all summary keys (columns 0..K-1).
      - if causal_context=False:
          allow ALL context keys (columns K..K+N-1).
        else (causal_context=True):
          for context_i (row K+i), allow only context keys K..K+i,
          block context keys > K+i.

    Args:
      num_summary:    K
      num_context:    N
      attend_counts:  length K-1, attend_counts[i-1] = how many context tokens summary_i may see.
      causal_context: if True, context tokens attend only to their own past (causally).

    Returns:
      mask: BoolTensor of shape (K+N, K+N), where True=blocked, False=allowed.
    """
    K, N = num_summary, num_context
    assert len(attend_counts) == K - 1
    ...
    # --- summary queries ---
    for i in range(K):
        # allow all summary keys
        mask[i, :K] = False

        # allow context keys per attend_counts
        if i == 0:
            # summary_0 sees all context
            mask[i, K:] = False
        else:
            cnt = attend_counts[i - 1]
            assert 0 <= cnt <= N
            mask[i, -cnt:] = False

    # --- context queries ---
    # block summary keys for all context rows
    mask[K:, :K] = True

    if not causal_context:
        # allow full context-to-context
        mask[K:, K:] = False
    else:
        # causal: for each context_i (row K+i), allow keys K..K+i
        for i in range(N):
            # row = K + i
            # columns K..K+i allowed
            mask[K + i, K:K + i + 1] = False
            # columns beyond K + i remain masked
    return mask
...
class DiffusionUnetHybridImageTargetedPolicy(BaseImagePolicy):
    def __init__(self, 
            ...
            num_cls_tokens=1, 
            attend_counts: List[int] = [],
            global_cond_ranges: List[Tuple[int, int]] = [],
        ...
        if num_cls_tokens > 1: 
            assert len(attend_counts) == num_cls_tokens - 1
            assert len(global_cond_ranges) == num_cls_tokens
        ...
        if num_cls_tokens > 1: 
            classifier_mask = make_summary_context_mask(
                num_summary=num_cls_tokens, 
                num_context=self.n_obs_steps, 
                attend_counts=attend_counts, 
                causal_context=causal_transformer
            )
            self.register_buffer("classifier_mask", classifier_mask)

            self.global_cond_ranges = global_cond_ranges
            starts, ends = zip(*global_cond_ranges)

            self.register_buffer('gc_start', torch.tensor(starts))
            self.register_buffer('gc_end', torch.tensor(ends))
        else: 
    ...
    def conditional_sample(self, 
        ...
        for t in scheduler.timesteps:
            ...
            if self.num_cls_tokens > 1: 
                in_range = (t >= self.gc_start) & (t <= self.gc_end)
                idx = in_range.float().argmax()

                batch_idx = torch.arange(batch_size, device=t.device)
                global_cond_to_pass = global_cond[batch_idx, idx, :]

            model_output = model(trajectory, t, 
                local_cond=local_cond, global_cond=global_cond_to_pass,
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor], use_DDIM=False) -> Dict[str, torch.Tensor]:
        ...
        if self.obs_as_global_cond:
            ...
            global_cond = self.resnet_post_processer(global_cond, mask=self.classifier_mask)
    ...
    def forward(self, batch, noisy_trajectory, timesteps):
        ...
        # Predict the noise residual
        if self.num_cls_tokens > 1: 
            in_range = (timesteps[:, None] >= self.gc_start[None, :]) & \
                (timesteps[:, None] <= self.gc_end[None, :])
            idx = in_range.float().argmax(dim=1)

            batch_idx = torch.arange(batch_size, device=timesteps.device)
            global_cond = global_cond[batch_idx, idx, :]
