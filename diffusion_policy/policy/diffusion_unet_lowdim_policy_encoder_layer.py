...
class DiffusionUnetLowdimPolicy(BaseLowdimPolicy):
    def __init__(self, 
        ...
        print("obs encoder params: %e" % sum(p.numel() for p in self.obs_encoder.parameters()))
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor], sample_for_vis: bool = False) -> Dict[str, torch.Tensor]:
        ...
        if self.obs_as_local_cond:
            ...
            obs_for_encoder = nobs[:,:To, :].reshape(-1, *nobs.shape[2:])
            global_cond = self.obs_encoder(obs_for_encoder)
    ...
    def compute_loss(self, batch):
        ...
        if self.obs_as_local_cond:
            ...
            obs_for_encoder = obs[:,:slice_end, :].reshape(-1, *obs.shape[2:])
            global_cond = self.obs_encoder(obs_for_encoder)
            global_cond = global_cond.reshape(obs.shape[0], -1)
            # breakpoint()
