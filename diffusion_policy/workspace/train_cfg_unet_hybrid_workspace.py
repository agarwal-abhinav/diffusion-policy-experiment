...
class TrainCfgUnetHybridWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        # Classifier-free guidance conditioning mask
        self.mask_flags = make_mask_flags(
            obs_shape=cfg.shape_meta['obs'],
            mask_images=cfg.training.mask_images,
            mask_past_actions=cfg.training.mask_past_actions,
            mask_target=cfg.training.mask_target
        )
            ...
            for local_epoch_idx in range(resume_epoch, cfg.training.num_epochs):
                ...
                with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}", 
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        ...
                        if random.uniform(0, 1) < cfg.training.p_uncond:
                            batch = make_uncond_batch(batch, self.mask_flags)
                ...
                # run diffusion (cfg) sampling on a _single_ validation batch from each dataset
