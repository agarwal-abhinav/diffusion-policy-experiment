"""
Usage:
python maze_eval/maze_image_eval.py --checkpoint <checkpoint> \
    -o data/eval_output/`date +"%Y.%m.%d"`-`date +"%H.%M.%S"`/maze_image_4000_eval_output
"""
...
def main(checkpoint, output_dir, device):
    ...
    with torch.no_grad():
        ...
        for i in tqdm.tqdm(range(num_traj)):
            ...
            maze = environment_generator.generate_maze_environment()
            regions = maze.regions
            bounds = maze.bounds

            source = maze.sample_start_point()
            ...
            while len(waypoints) <= 300:
                obs_dict = deque_to_dict(state_deque, img_deque, target)
...
def deque_to_dict(state_deque: deque, img_deque: deque, target: np.ndarray) -> dict[str, torch.Tensor]:
    state_array = torch.cat(list(state_deque), axis=1) # 1, T_obs, D_x
    img_array = torch.cat(list(img_deque), axis=1) # 1, T_obs, C, H, W
    target = torch.from_numpy(target).reshape(1,-1) # 1, D_t
    return {'obs': {
                'image': img_array, # 1, T_obs, C, H, W
                'agent_pos': state_array, # 1, T_obs, D_x
            },
            'target': target, # 1, D_t
    }
