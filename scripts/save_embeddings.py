...
def save_actions_and_embeddings(
    ...
    embedding_dim = n_obs_steps * embedding_dim

    # Compute embeddings
    num_real = len(datasets['real'])
    real_actions = torch.zeros((num_real, 16, 2))
    real_embeddings = torch.zeros((num_real, embedding_dim))
    with torch.no_grad():
    ...
    sim_embeddings = torch.zeros((num_sim, embedding_dim))
    with torch.no_grad():
